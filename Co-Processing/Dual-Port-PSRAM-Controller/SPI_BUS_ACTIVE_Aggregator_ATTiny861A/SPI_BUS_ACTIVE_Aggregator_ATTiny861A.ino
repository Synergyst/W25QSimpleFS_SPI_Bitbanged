...
#include <Arduino.h>
const uint8_t CS_MASK = 0xFF;     // Inputs: CS_n lines on Port A (use PA0..PA7)
const uint8_t PIN_BUS_OUT = PB3;  // Output: BUS_ACTIVE (active-HIGH)
...
void loop() {
  ...
  delayMicroseconds(1);  // Optional tiny glitch filter (comment out if not needed)
}