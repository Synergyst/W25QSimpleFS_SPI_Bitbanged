...
public:
  ...
  void readId9F(uint8_t& mid, uint8_t& did1, uint8_t& did2, uint8_t& dummyOut) {
    ...
    b0 = MX35_SPI_INSTANCE.transfer((uint8_t)0x00);
    b1 = MX35_SPI_INSTANCE.transfer((uint8_t)0x00);
    b2 = MX35_SPI_INSTANCE.transfer((uint8_t)0x00);
    b3 = MX35_SPI_INSTANCE.transfer((uint8_t)0x00);
    endTx();
    csHigh();
    dummyOut = b0;
    mid = b1;
  ...
  bool identify(IdInfo& info) {
    ...
    if (mid == 0xC2) {
      switch (did1) {
        case 0x12: info.totalBytes = (uint64_t)128 * 1024 * 1024; return true;
        case 0x22: info.totalBytes = (uint64_t)256 * 1024 * 1024; return true;
        case 0x2C: info.totalBytes = (uint64_t)512 * 1024 * 1024; return true;
        case 0x26: info.totalBytes = (uint64_t)256 * 1024 * 1024; return true;
        case 0x37: info.totalBytes = (uint64_t)512 * 1024 * 1024; return true;
        default: break;
      }
    }
    info.totalBytes = 0;
...
public:
  ...
  bool identifyCS(uint8_t cs, DeviceInfo& out, uint32_t spiHzForId = UNIFIED_SPI_CLOCK_HZ) {
    ...
    // 2) MX35 SPI-NAND (accept only if identify() maps capacity and confirms Macronix)
    {
      if (_wp >= 0) {
        pinMode(_wp, OUTPUT);
        digitalWrite(_wp, HIGH);
      ...
      mx.reset();
      delay(2);
      uint8_t mid = 0, did1 = 0, did2 = 0, dum = 0;
      mx.readId9F(mid, did1, did2, dum);
      if (mid == 0x00 || mid == 0xFF) {
        uint8_t dmy = 0;
        mx.readId90(did1, did2, dmy);
        if (did1 != 0x00 && did1 != 0xFF) mid = 0xC2;
      }
