...
// Data memory barrier helper (Cortex-M0+)
...
public:
  ...
  void core1Poll() {
    ...
    // Ensure we see producer's job fields before we indicate 'running'
    exechost_dmb();
    __asm volatile("dsb" ::
                     : "memory");
    __asm volatile("isb" ::
  ...
  void pollBackground() {
    if (!_bg_active) {
      if (_job_flag == 3u) {
        // Clear done flag after we've observed it
        exechost_dmb();
        __asm volatile("dsb" ::
                         : "memory");
        __asm volatile("isb" ::
    ...
    if (_job_flag == 3u) {
      // Ensure we see result/status before using
      exechost_dmb();
