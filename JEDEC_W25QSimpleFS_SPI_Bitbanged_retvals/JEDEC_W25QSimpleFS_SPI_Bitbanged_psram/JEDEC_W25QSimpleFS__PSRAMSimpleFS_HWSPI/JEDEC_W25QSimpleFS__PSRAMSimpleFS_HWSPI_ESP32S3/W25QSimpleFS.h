...
public:
  ...
  // Backward-compatible ctor for W25QBitbang
  explicit W25QSimpleFS(W25QBitbang& flash) {
    bind<W25QBitbang>(flash);
  }

  // Generic ctor: accept any driver D exposing:
  ...
  bool mount(bool autoFormatIfEmpty = true) {
    _capacity = 0;
    uint8_t m, t, c;
    _capacity = _ops.jedec(_ops.ctx, m, t, c);
    ...
    for (uint32_t i = 0; i < dirEntries; ++i) {
      ...
      _ops.read(_ops.ctx, addr, buf, ENTRY_SIZE);
  ...
  bool wipeChip() {
    bool ok = _ops.chipErase(_ops.ctx);
    if (ok) {
      _fileCount = 0;
      _dirWriteOffset = 0;
      _nextSeq = 1;
  ...
  bool writeFile(const char* name, const uint8_t* data, uint32_t size,
    ...
    if (size > 0 && !_ops.prog(_ops.ctx, start, data, size)) return false;
  ...
  bool createFileSlot(const char* name, uint32_t reserveBytes, const uint8_t* initialData = nullptr, uint32_t initialSize = 0) {
    ...
    if (initialSize > 0) {
      if (!_ops.prog(_ops.ctx, start, initialData, initialSize)) return false;
    }
  ...
  bool writeFileInPlace(const char* name, const uint8_t* data, uint32_t size, bool allowReallocate = false) {
    ...
    if (fi.slotSafe && cap >= size) {
      ...
      if (size > 0 && !_ops.prog(_ops.ctx, fi.addr, data, size)) return false;
  ...
  uint32_t readFile(const char* name, uint8_t* buf, uint32_t bufSize) {
    ...
    if (n == 0) return 0;
    _ops.read(_ops.ctx, _files[idx].addr, buf, n);
    return n;
  }
  uint32_t readFileRange(const char* name, uint32_t offset, uint8_t* buf, uint32_t len) {
    ...
    if (len == 0) return 0;
    _ops.read(_ops.ctx, _files[idx].addr + offset, buf, len);
    return len;
  }
...
private:
  ...
  struct FlashOps {
    void* ctx = nullptr;
    uint32_t (*jedec)(void*, uint8_t&, uint8_t&, uint8_t&) = nullptr;
    void (*read)(void*, uint32_t, uint8_t*, uint32_t) = nullptr;
    bool (*prog)(void*, uint32_t, const uint8_t*, uint32_t) = nullptr;
    bool (*erase4k)(void*, uint32_t) = nullptr;
    bool (*chipErase)(void*) = nullptr;
  } _ops;

  template<typename D>
  static uint32_t jedecThunk(void* ctx, uint8_t& m, uint8_t& t, uint8_t& c) {
    return static_cast<D*>(ctx)->readJEDEC(m, t, c);
  }
  template<typename D>
  static void readThunk(void* ctx, uint32_t addr, uint8_t* buf, uint32_t len) {
    static_cast<D*>(ctx)->readData(addr, buf, len);
  }
  template<typename D>
  static bool progThunk(void* ctx, uint32_t addr, const uint8_t* data, uint32_t len) {
    return static_cast<D*>(ctx)->pageProgram(addr, data, len);
  }
  template<typename D>
  static bool erase4kThunk(void* ctx, uint32_t addr) {
    return static_cast<D*>(ctx)->sectorErase4K(addr);
  }
  template<typename D>
  static bool chipEraseThunk(void* ctx) {
    return static_cast<D*>(ctx)->chipErase();
  }
  template<typename D>
  void bind(D& d) {
    _ops.ctx = &d;
    _ops.jedec = &jedecThunk<D>;
    _ops.read = &readThunk<D>;
    _ops.prog = &progThunk<D>;
    _ops.erase4k = &erase4kThunk<D>;
    _ops.chipErase = &chipEraseThunk<D>;
  }
  ...
  bool appendDirEntry(uint8_t flags, const char* name, uint32_t addr, uint32_t size) {
    ...
    _ops.read(_ops.ctx, DIR_START + _dirWriteOffset, existing, ENTRY_SIZE);
    ...
    bool ok = _ops.prog(_ops.ctx, DIR_START + _dirWriteOffset, rec, ENTRY_SIZE);
    if (!ok) return false;
  ...
  bool eraseRange(uint32_t start, uint32_t len) {
    ...
    for (uint32_t s = a; s < end; s += SECTOR_SIZE) {
      ...
      if (!_ops.erase4k(_ops.ctx, s)) return false;
      yield();
    }
    return true;
  }
