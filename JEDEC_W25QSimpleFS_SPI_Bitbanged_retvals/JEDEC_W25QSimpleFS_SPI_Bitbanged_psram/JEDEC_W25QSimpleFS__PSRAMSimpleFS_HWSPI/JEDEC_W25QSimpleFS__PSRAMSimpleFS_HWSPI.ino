/*
  ...
  Integrated program (HW SPI bus):
   - RP2040 + W25Q flash via HW SPI (W25QSimpleFS + W25QHwSPI)
   - Optional PSRAM via HW SPI (PSRAMSimpleFS + PSRAMHwSPI)
   ...
   - TFT ST7789 2.0" SPI display support via Adafruit library (GMT020-02 style)
  ...
  Notes:
   ...
   - All devices share the same MOSI/MISO/SCK hardware SPI bus; each has a unique CS pin.
*/
...
// HW SPI drivers (replace these includes/class names if your library uses different names)
#include "W25QHwSPI.h"
#include "PSRAMHwSPI.h"
...
// ========== Shared HW SPI bus pin definitions ==========
// Bus: SPI1 -> MISO=GP12, SCK=GP14, MOSI=GP15
const uint8_t PIN_SPI_MISO = 12;
const uint8_t PIN_SPI_SCK = 14;
const uint8_t PIN_SPI_MOSI = 15;

// ========== Per-device CS/control pins ==========
// Flash (W25Q)
const uint8_t PIN_FLASH_CS = 13;  // GP13 (unique CS)
// PSRAM
const uint8_t PIN_PSRAM_CS = 9;          // GP9  (unique CS)
const bool PSRAM_ENABLE_QPI = false;     // keep false unless you add IO2/IO3 and HW supports it
const uint8_t PSRAM_CLOCK_DELAY_US = 0;  // not used by HW driver; kept for compatibility if needed

// ========== TFT display pins (HW SPI via Adafruit ST7789) ==========
// Wiring for GMT020-02 2.0" TFT SPI on WaveShare RP2040 Zero (shared MOSI/SCK):
//   SCK -> shared SPI SCK
//   MOSI -> shared SPI MOSI
//   RST -> GP3
//   DC  -> GP2
//   CS  -> GP5
const uint8_t PIN_TFT_SCK = PIN_SPI_SCK;    // shared SCK
const uint8_t PIN_TFT_MOSI = PIN_SPI_MOSI;  // shared MOSI
const uint8_t PIN_TFT_RST = 3;              // RST pin
const uint8_t PIN_TFT_DC = 2;               // DC pin
const uint8_t PIN_TFT_CS = 5;               // CS pin
// MISO is unused by ST7789; it will be the shared bus MISO but TFT does not drive it.
...
// Adafruit ST7789 instance on SPI1 (hardware SPI)
static Adafruit_ST7789 tft = Adafruit_ST7789(&SPI1, PIN_TFT_CS, PIN_TFT_DC, PIN_TFT_RST);
...
// HW SPI drivers: pass SPI bus and CS pin
W25QHwSPI flashHW(SPI1, PIN_FLASH_CS);
W25QSimpleFS fsFlash(flashHW);

PSRAMHwSPI psramHW(SPI1, PIN_PSRAM_CS);
constexpr uint32_t PSRAM_CAPACITY_BYTES = 8UL * 1024UL * 1024UL;
PSRAMSimpleFS fsPSRAM(psramHW, PSRAM_CAPACITY_BYTES);
...
// ===== TFT API (now using Adafruit_ST7789 over HW SPI) =====
...
static bool tftInit(uint8_t rotation) {
  ...
  // Bump SPI to a high rate (Adafruit library manages transactions per device)
  tft.setSPISpeed(80000000);  // adjust if your wiring requires lower
...
static void printHelp() {
  ...
  Console.println("TFT wiring: SCK->GP14, MOSI->GP15, DC->GP2, CS->GP5, RST->GP3 (shared MOSI/SCK bus)");
  Console.println();
}
...
void setup() {
  ...
  // Configure shared SPI1 pins and start the bus
  SPI1.setRX(PIN_SPI_MISO);  // RP2040 ZERO example mapping: pin 12
  SPI1.setSCK(PIN_SPI_SCK);  // e.g., pin 14
  SPI1.setTX(PIN_SPI_MOSI);  // e.g., pin 15
  SPI1.begin();

  // Ensure all CS are idle-high before device init
  pinMode(PIN_FLASH_CS, OUTPUT);
  digitalWrite(PIN_FLASH_CS, HIGH);
  pinMode(PIN_PSRAM_CS, OUTPUT);
  digitalWrite(PIN_PSRAM_CS, HIGH);
  pinMode(PIN_TFT_CS, OUTPUT);
  digitalWrite(PIN_TFT_CS, HIGH);

  // Initialize flash and PSRAM HW SPI drivers
  flashHW.begin();
  psramHW.begin();
  // If your PSRAM HW driver supports mode settings, configure here; we keep default SPI mode/clock.
  ...
  // Initialize TFT over shared HW SPI bus
  tftInit(tft_rot);
